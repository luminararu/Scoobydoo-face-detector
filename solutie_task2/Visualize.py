...
def show_detections_with_ground_truth(detections, scores, file_names, params: Parameters, personaj):
    """
    Afiseaza si salveaza imaginile adnotate. Deseneaza bounding box-urile prezice si cele corecte.
    detections: numpy array de dimensiune NX4, unde N este numarul de detectii pentru toate imaginile.
    detections[i, :] = [x_min, y_min, x_  max, y_max]
    scores: numpy array de dimensiune N, scorurile pentru toate detectiile pentru toate imaginile.
    file_names: numpy array de dimensiune N, pentru fiecare detectie trebuie sa salvam numele imaginii.
    (doar numele, nu toata calea).
    """

    np.save(os.path.join(params.dir_save_files, f'detections_all_faces_{personaj}.npy'), detections)
    np.save(os.path.join(params.dir_save_files, f'scores_all_faces_{personaj}.npy'), scores)
    np.save(os.path.join(params.dir_save_files, f'file_names_all_faces_{personaj}.npy'), file_names)
    print(f'Salvat {len(detections)} detecții în {params.dir_save_files}')


    params.path_annotations = os.path.join(params.base_dir_valid, f'task2_{personaj}_gt_validare.txt')
...
def salveaza_detectiile(detections, scores, file_names, params: Parameters, personaj):
    np.save(os.path.join(params.dir_save_files, f'detections_all_faces_{personaj}.npy'), detections)
    np.save(os.path.join(params.dir_save_files, f'scores_all_faces_{personaj}.npy'), scores)
    np.save(os.path.join(params.dir_save_files, f'file_names_all_faces_{personaj}.npy'), file_names)
